...
const (
	_Max     byte = 0x7F
	_Unknown rune = '?'
	_Esc     byte = 0x1B // Esc code
	_Sub     byte = 0x1A // Ctrl+Z code
	_CR      byte = 0x0D // <CR> code
)

var (
	crcr       = []byte{_CR, _CR}
	cr         = []byte{_CR}
	gsmEscapes = escapeTable{
...
// Encode7bit interface
type Encode7bit interface {
	...
	DisplayPack([]byte) string
}

type impl7bit struct {
}
